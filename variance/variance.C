...
gpT->SetLineStyle(9);
gpT->SetTitle("Standardized Variance");
gpT->GetXaxis()->SetTitle("Multiplicity Classes");
gpT->GetYaxis()->SetTitle("#sigma_{standardized}");
gpT->Draw("ALP");

auto legend = new TLegend(0.1,0.7,0.36,0.9);
   legend->SetHeader("Legend"); // option "C" allows to center the header
   legend->AddEntry("gpT","Standardised Variance for corresponding multiplicity class","P");
